...
.timeline::before {
    content: '';
    position: absolute;
    left: .9rem; top: 1.5rem; bottom: 1.5rem;
    width: 2px; background: #e3e7ef;
    border-radius: 2px;
}
.tl-item {
    position: relative;
...
.tl-dot {
    ...
    border-radius: 50%; border: 2px solid #e3e7ef;
    background: #f8fafc;
    display: flex; align-items: center; justify-content: center;
    font-size: .65rem; color: #94a3b8; transition: all .3s;
}
.tl-item.done .tl-dot    { background: #16a34a; border-color: #16a34a; color: #fff; }
.tl-item.active .tl-dot  { background: #0b5ed7; border-color: #0b5ed7; color: #fff; animation: pulse 1.5s infinite; }
.tl-item.pending .tl-dot { background: #f8fafc; border-color: #cbd5e1; color: #94a3b8; }
@keyframes pulse {
    0%, 100% { box-shadow: 0 0 0 0 rgba(11,94,215,.3); }
    50%       { box-shadow: 0 0 0 6px rgba(11,94,215,0); }
}
.tl-title { font-size: .9rem; font-weight: 600; color: #1e293b; margin-bottom: .15rem; }
.tl-title.muted { color: #94a3b8; }
.tl-time  { font-size: .78rem; color: #94a3b8; }

/* ── Detail table ── */
.detail-row { display: flex; justify-content: space-between; padding: .45rem 0; border-bottom: 1px solid #f1f5f9; font-size: .875rem; }
...
@php
    ...
    $steps = [
        ['key'=>'pendiente', 'label'=>'Pedido recibido',    'icon'=>'✓', 'desc'=>'Tu pedido fue registrado en el sistema.'],
        ['key'=>'en_camino', 'label'=>'En camino',           'icon'=>'🚚','desc'=>'Tu repartidor está en ruta hacia tu ubicación.'],
        ['key'=>'entregado', 'label'=>'Entregado',           'icon'=>'✓', 'desc'=>'Tu pedido fue entregado correctamente.'],
    ];
@endphp

<div class="page-header" style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:.75rem;">
    <div>
        <h1>Seguimiento del Pedido</h1>
        <p>
            <span style="font-family:monospace; font-weight:700; color:#1e40af; font-size:1rem;">{{ $order->codigo_orden }}</span>
        </p>
    </div>
    ...
    <div class="p-card">
        ...
        <div class="timeline">
            ...
            <div class="tl-item {{ $cls }}">
                <div class="tl-dot">
                    @if($isDone)<i class="ri-check-line" style="font-size:.8rem;"></i>
                    @elseif($isActive)<i class="ri-loader-4-line ri-spin" style="font-size:.8rem;"></i>
                    @else<span>{{ $idx + 1 }}</span>@endif
                </div>
                <div class="tl-title {{ !$isDone && !$isActive ? 'muted' : '' }}">{{ $step['label'] }}</div>
                @if($isDone || $isActive)
                    <div class="tl-time">{{ $step['desc'] }}</div>
                    @if($isActive && $order->repartidor)
                        <div style="margin-top:.5rem; display:inline-flex; align-items:center; gap:.4rem; background:#eff6ff; border:1px solid #bfdbfe; border-radius:8px; padding:.35rem .75rem; font-size:.82rem; color:#1d4ed8; font-weight:600;">
                            <i class="ri-user-line"></i> Repartidor: {{ $order->repartidor->nombres ?? 'Asignado' }}
                        </div>
                    @endif
                @else
        ...
        <div style="text-align:center; padding:2rem; color:#94a3b8;">
            <i class="ri-close-circle-line" style="font-size:3rem; color:#fca5a5; display:block; margin-bottom:.75rem;"></i>
            <p style="font-weight:600; color:#dc2626; margin:0;">Pedido cancelado</p>
        </div>
        @endif

        @if($order->notas)
            <div style="margin-top:1.5rem; background:#fffbeb; border:1px solid #fde68a; border-radius:10px; padding:.85rem 1rem;">
                <div style="font-size:.78rem; font-weight:700; text-transform:uppercase; letter-spacing:.04em; color:#92400e; margin-bottom:.35rem;">
    ...
    <div style="display:flex; flex-direction:column; gap:1rem;">
        ...
        <div class="p-card">
            ...
            <div class="detail-row">
                ...
                <span class="detail-value">{{ $order->fecha_programada ? \Carbon\Carbon::parse($order->fecha_programada)->format('d/m/Y') : '—' }}</span>
            </div>
            <div class="detail-row">
        ...
        <div class="p-card">
            ...
            <div class="detail-row">
                ...
                <span class="detail-value" style="max-width:180px; word-wrap:break-word; text-align:right;">{{ $order->direccion_entrega }}</span>
            </div>
            @if($order->referencia)
            <div class="detail-row">
                <span class="detail-label">Referencia</span>
                <span class="detail-value" style="max-width:180px; word-wrap:break-word; text-align:right;">{{ $order->referencia }}</span>
            </div>
            @endif
            <div class="detail-row">
